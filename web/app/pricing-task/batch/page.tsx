...
export default function PricingTaskBatchListPage() {
  ...
  async function bootstrap() {
    setLoading(true)
    try {
      setBatches(await fetchPricingTaskBatches())
    } finally {
      setLoading(false)
    }
  }
  ...
  async function openModal() {
    ...
    try {
      ...
      if (!batchName) setBatchName(`批量组价 ${new Date().toLocaleString()}`)
    } finally {
  ...
  async function createBatch() {
    if (!batchName.trim() || !selectedProject) {
      alert('请填写批次名称并选择工程')
      return
    }
    setSaving(true)
    try {
      const created = await createPricingTaskBatch({
        name: batchName.trim(),
      ...
      setShowModal(false)
      router.push(`/pricing-task/batch/${created.id}`)
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">批量组价</h1>
                ...
                {batches.map(batch => (
                  <tr key={batch.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link href={`/pricing-task/batch/${batch.id}`} className="mr-3 text-blue-600 hover:text-blue-700">
                        进入
                      </Link>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="mx-4 w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
              <h2 className="mb-6 text-xl font-bold text-gray-900">新建批量组价批次</h2>
              {modalLoading ? (
