...
import {
  QuotaInputPromptItem,
  QuotaTreeCategory,
  QuotaTreeItemDetail,
  fetchQuotaInputPrompts,
  fetchQuotaTreeItemDetail,
  fetchQuotaTreeItemDetailByCode,
  ...
  return (
    <aside className="border-b border-gray-200 bg-white lg:border-b-0 lg:border-r">
      <div className="border-b border-gray-200 px-4 py-3">
        ...
        <div className="mt-0.5 text-xs text-gray-400">按 DEKID 筛选实际值提示</div>
      </div>
        ...
        {detail && (
          <div className="space-y-4 p-5">
            ...
            <section>
              ...
              <div className="grid grid-cols-2 border-l border-t border-gray-200 bg-white sm:grid-cols-3 xl:grid-cols-4">
                {COST_LABELS.map(([key, label]) => (
                  <div key={key} className="border-b border-r border-gray-200 px-3 py-2">
            ...
            {(detail.input_prompt_rules?.length ?? 0) > 0 && (
              <section>
                ...
                <PromptRuleTable rules={detail.input_prompt_rules} dekid={detail.item.dekid} onOpenAdjustment={undefined} />
              </section>
            )}
...
function PromptRuleTable({
  rules,
  dekid,
  onOpenAdjustment,
}: {
  rules: QuotaInputPromptItem['prompt_rules']
  dekid: number
  onOpenAdjustment?: (dekid: number, code: string) => void
}) {
  return (
    <div className="overflow-x-auto border border-gray-200">
      <table className="min-w-full divide-y divide-gray-200 text-xs">
        <thead className="bg-gray-50 text-gray-500">
          <tr>
            <th className="px-3 py-2 text-left font-medium">实际值提示</th>
        ...
        <tbody className="divide-y divide-gray-100 bg-white">
          {rules.map((rule, index) => (
            <tr key={`${dekid}:${rule.adjustment_code ?? ''}:${index}`}>
              <td className="px-3 py-2 text-gray-800">{rule.prompt ?? '-'}</td>
              <td className="px-3 py-2">
                {rule.adjustment_code && onOpenAdjustment ? (
                  <button
                    type="button"
                    onClick={() => onOpenAdjustment(dekid, rule.adjustment_code!)}
                    className="font-mono font-semibold text-blue-700 hover:underline"
                    title="查看关联换算定额详情"
                  >
                    {rule.adjustment_code}
                  </button>
                ) : (
                  <span className="font-mono text-blue-700">{rule.adjustment_code ?? '-'}</span>
                )}
              </td>
              <td className="px-3 py-2 text-right font-mono text-gray-900">{rule.base_value ?? '-'}</td>
              <td className="px-3 py-2 text-right font-mono text-gray-900">{rule.increment_unit ?? '-'}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default function QuotaInputPromptsPage() {
  ...
  const [items, setItems] = useState<QuotaInputPromptItem[]>([])
  ...
  const libraryPromptTotal = useMemo(
    () => items.reduce((sum, item) => sum + item.prompt_count, 0),
    [items],
  )
  ...
  const loadData = useCallback(async (nextPage: number, query: string, libraryId: number | null) => {
    ...
    setError('')
    try {
      const result = await fetchQuotaInputPrompts({
      ...
      setError(reason instanceof Error ? reason.message : '加载实际值提示失败')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  async function openQuotaDetailByCode(dekid: number, code: string) {
    setDetailOpen(true)
    setDetail(null)
    setDetailError('')
    setDetailLoading(true)
    try {
      const result = await fetchQuotaTreeItemDetailByCode(dekid, code)
      setDetail(result)
    } catch (reason) {
      setDetailError(reason instanceof Error ? reason.message : '加载关联换算定额失败')
    } finally {
      setDetailLoading(false)
    }
  }
      ...
      <div className="grid min-h-[calc(100vh-220px)] lg:grid-cols-[320px_minmax(0,1fr)]">
        ...
        <main className="min-w-0 p-4 sm:p-5">
          <div className="mb-4 border border-gray-200 bg-white px-4 py-3">
            ...
            <div className="mt-1 text-xs text-gray-500">
              按定额库分页查看存在实际值提示的定额；点击文本可直接选择复制。
            </div>
          </div>
          ...
          <div className="overflow-hidden border border-gray-200 bg-white">
            ...
            {!loading && items.length === 0 && (
              <div className="px-4 py-12 text-center text-sm text-gray-400">暂无实际值提示数据</div>
            )}

            <div className="divide-y divide-gray-100">
              {items.map(item => (
                <section key={`${item.dekid}:${item.quota_item_id}`} className="px-4 py-3">
                  <div className="grid gap-3 text-sm xl:grid-cols-[150px_minmax(260px,1fr)_90px] xl:items-start">
                    <div>
                      {item.quota_code ? (
                        <button
                          type="button"
                  ...
                  <div className="mt-3">
                    <PromptRuleTable rules={item.prompt_rules} dekid={item.dekid} onOpenAdjustment={openQuotaDetailByCode} />
                  </div>
                </section>
              ))}
            </div>
