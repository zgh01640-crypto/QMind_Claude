...
export default function NewBoqListPage() {
  ...
  const doUpload = async (force = false) => {
    ...
    try {
      const proj = await uploadBoqFile(pendingFile, force, projectName.trim())
      setProjects(prev => [proj, ...prev.filter(p => p.id !== proj.id)])
      setShowUploadDialog(false)
      setPendingFile(null)
      router.push(`/new-boq/${proj.id}`)
    } catch (e: unknown) {
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={uploading}
            className="px-4 py-2 bg-indigo-700 text-white text-sm rounded hover:bg-indigo-800 disabled:opacity-50 flex items-center gap-2"
          >
            {uploading
      ...
      {projects.length === 0 && !loading && (
        <div
          ...
          className={`border-2 border-dashed rounded-xl py-20 text-center cursor-pointer transition-colors mb-6 ${
            drag ? 'border-indigo-400 bg-indigo-50' : 'border-gray-300 hover:border-indigo-300 hover:bg-gray-50'
          }`}
        >
      ...
      {projects.length > 0 && (
        <div
          ...
          className={`mt-6 border border-dashed rounded-lg py-4 text-center text-sm transition-colors ${
            drag ? 'border-indigo-400 bg-indigo-50 text-indigo-600' : 'border-gray-200 text-gray-400'
          }`}
        >
          拖拽文件到此处导入新工程
        </div>
      ...
      {showUploadDialog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full mx-4 p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">新增工程</h2>
            <div className="mb-4">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-indigo-700 text-white rounded-lg hover:bg-indigo-800 disabled:opacity-50 flex items-center gap-2"
              >
                {uploading
                  ? <><span className="animate-spin inline-block w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full" />上传中…</>
                  : '上传'}
